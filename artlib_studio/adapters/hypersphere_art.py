"""Hypersphere ART adapter for ARTLib Studio."""
...
class HypersphereARTAdapter(ARTModelAdapter):
    @property
    def name(self) -> str:
        return "Hypersphere ART"

    @property
    def model_key(self) -> str:
        return "hypersphere_art"
    ...
    def capabilities(self) -> Set[Capability]:
        return {
            ...
            Capability.HYPERSPHERE_REGIONS_2D,
    ...
    def create_model(self, params: dict) -> Any:
        return HypersphereART(
            rho=float(params.get("rho", 0.75)),
            alpha=float(params.get("alpha", 0.0001)),
            beta=float(params.get("beta", 1.0)),
            r_hat=float(params.get("r_hat", 1.0))
        )
    ...
    def param_schema(self) -> dict:
        return {
            "rho": {"type": "float", "min": 0.0, "max": 1.0, "default": 0.75, "step": 0.01, "label": "vigilance (rho)"},
            "alpha": {"type": "float", "min": 0.0, "max": 10.0, "default": 0.0001, "step": 0.01, "label": "choice (alpha)"},
            "beta": {"type": "float", "min": 0.0, "max": 1.0, "default": 1.0, "step": 0.01, "label": "learning rate (beta)"},
            "r_hat": {"type": "float", "min": 0.0, "max": 20.0, "default": 1.0, "step": 0.1, "label": "max radius (r_hat)"},
        }

    def fit_with_trace(self, X: Any, params: dict, recorder: Optional[TraceRecorder] = None) -> Any:
        model = self.create_model(params)
        instr = InstrumentedART(model, recorder=recorder, prepare_data=False)
        instr.fit(X)
        return instr
    ...
    def get_category_geometry_2d(self, model: Any) -> Any:
        """Return hyperspheres (centroid and radius)."""
        spheres = []
        if hasattr(model, "W"):
            for idx, w in enumerate(model.W):
                # HypersphereART weight is [centroid_X, ..., radius]
                centroid = w[:-1]
                radius = w[-1]
                if len(centroid) >= 2:
                    spheres.append({
